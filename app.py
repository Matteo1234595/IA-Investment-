...
if menu == "Dashboard":
    ...
    for i, (nome, symbol) in enumerate(indices.items()):
        ...
        delta = data["Close"].iloc[-1] - data["Close"].iloc[0]
        pct = (delta / data["Close"].iloc[0]) * 100
        if i % 2 == 0:
            col1.metric(label=nome, value=f"{data['Close'].iloc[-1]:.2f}", delta=f"{pct:+.2f}%")
        else:
            col2.metric(label=nome, value=f"{data['Close'].iloc[-1]:.2f}", delta=f"{pct:+.2f}%")
